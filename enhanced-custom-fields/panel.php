...
class PECF_Panel {
	 ...
	 */
	function PECF_Panel($id, $title, $post_type, $context, $priority) {
		PECF_Panels_static_add_panel_id($id);
	    $this->id = $id;
	    $this->title = $title;
...
//	    $theme_root = get_bloginfo('stylesheet_directory');
	    $theme_root = plugins_url('', __FILE__);
	    wp_enqueue_style(
	    	'pecf_styles',
	    	"$theme_root/tpls/style.css"
	    );
	...
	function _attach() {
	    ...
	    //wp_enqueue_script('pecf', get_bloginfo('stylesheet_directory') . '/lib/enhanced-custom-fields/tpls/pecf.js');
	    wp_enqueue_script('pecf', plugins_url( '/tpls/pecf.js', __FILE__ ) . '');
	...
	function add_fields($fields) {
		foreach ($fields as $field) {
			if (!is_a($field, 'PECF_Field')) {
				pecf_conf_error("Trying to add non PECF object ot PECF panel field");
			}
			PECF_Panels_static_add_field_name($field->name);
		}
	    $this->fields =& $fields;
	}
	...
	function save($post_id) { 
		...
		if ( defined('DOING_AUTOSAVE') && DOING_AUTOSAVE ) {
			return;
		// or a gravity form
		} elseif ( isset($_POST['gform_submit']) && $_POST['gform_submit'] ) {
			return;
		}
...
// keep all PECF field names and notify the end user if duplicated field name is found
function PECF_Panels_static_add_field_name($name) {
	static $names;
	if (isset($names[$name])) {
		pecf_conf_error("Trying to add PECF Field with name <code>$name</code> twice.");
	}
    $names[$name] = 1;
}
function PECF_Panels_static_add_panel_id($panel_id) {
    static $ids;
	if (isset($ids[$panel_id])) {
		pecf_conf_error("Trying to add PECF Panel with ID <code>$panel_id</code> twice.");
	}
    $ids[$panel_id] = 1;
}
?>